...
- **The loan figure changes basis at CECL (~2020).** There is no universal "total loans" tag, and the tag the panel lands on flips at CECL adoption. **Before** CECL the panel sources a net-of-allowance tag (`LoansAndLeasesReceivableNetReportedAmount`) for the large majority of banks (135 of 138 at FY2016); **after** CECL it sources the `FinancingReceivable…BeforeAllowance` (gross) tag for the large majority (~106 banks at FY2024, of which 103 are gross and only 3 net). So `total_loans` — and `loan_deposit_ratio` built on it — carries a **within-series net→gross break around 2020 and is not comparable across that boundary**. (Three filers — JPMorgan, CUBI, THFF — read as *net* despite sitting on a before-allowance tag, because they populate the before/after-allowance concepts with inverted values; JPM's `total_loans` is net 1,299,590M = gross 1,323,643M − allowance 24,345M.) I flag this in full because it is honest — but it does not touch the result: **`total_loans` and `loan_deposit_ratio` are balance-sheet features I do not use in the reported analysis or its conclusions (the NII–M2–rate–bitcoin finding), so this basis break does not affect the findings.**

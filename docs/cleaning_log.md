...
- **Balance-sheet tag heterogeneity — `total_loans`** — no universal loans tag; canonical `LoansAndLeasesReceivableNetReportedAmount` (162 banks) with fallbacks `…NetOfDeferredIncome` (1), `NotesReceivableNet` (2), `FinancingReceivable…BeforeAllowance` (6, incl. JPM). **Mixes net-of-allowance and gross bases** — flagged in the data dictionary; treat `total_loans` (and derived `loan_deposit_ratio`) as approximate.
